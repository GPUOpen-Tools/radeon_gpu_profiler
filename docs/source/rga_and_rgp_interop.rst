...
Some pipeline binaries are associated with multiple shaders. In this case, the total number of shaders
for the pipeline will be listed in parentheses next to the pipeline hash. Pipelines can be
filtered using the text search bar in the top right. Multiple selections can be made
simultaneously by clicking and dragging the mouse up or down. Once a selection is made, pushing the
"Analyze selected binaries" button will save and open all binaries that were checked in the
list in RGA. Keep in mind opening a large number of pipeline binaries in RGA may take some time.
